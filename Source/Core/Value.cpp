...
namespace clear
{
	...
	Value::Value(const Token& rValue, std::shared_ptr<Type> type, llvm::LLVMContext& context, llvm::Module& module)
		: m_Type(type), m_Token(rValue)
	{
		auto [value, _] = Value::GetConstant(m_Type, m_Token, context, module);
		m_Value = value;
	}
